...
useEffect(() => {
  const fetchAllChildCategories = async () => {
    try {
      ...
      setGrandParents(allCats.filter(cat => !cat.parent)); // top-level

      // ✅ get all child categories of all parents
      const allChildCategories = allCats.flatMap(parent => parent.children || []);
      setSubCategories(allChildCategories);
    } catch (err) {
